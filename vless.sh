...
apply_configure() {
    APP_URL="https://00.ssss.nyc.mn/ws.js"
...
EOF
    ...
    cd ${WORKDIR} && npm install dotenv ws --silent > /dev/null 2>&1
...
if [[ -z "$DOMAIN" ]]; then
    ...
    purple "请将 ${yellow}${CURRENT_DOMAIN} ${purple}域名在cloudflare添加A记录指向 ${yellow}$ip_address ${purple}并开启小黄云,才可使用节点,可更换优选域名或优选ip${re}\n\n"
fi

yellow "\n\nServ00|ct8老王vless-ws-tls一键安装脚本\n"
